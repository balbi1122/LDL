...
function setupInputListeners() {
    ...
    rangeInputs.forEach(function(slider) {
        ...
        // Set initial value for the number input (display formatted)
        const initialValue = parseFloat(slider.value);
        numberInput.value = slider.id === 'loanTerm' ? initialValue : formatCurrency(initialValue);
        console.log("Set initial value for", slider.id, ":", numberInput.value);
        
        // Update number input when slider changes
        slider.addEventListener('input', function() {
            const value = parseFloat(slider.value);
            numberInput.value = slider.id === 'loanTerm' ? value : formatCurrency(value);
...
function calculateAndDisplayResults() {
    // Get values directly from the range inputs (not the formatted display inputs)
...
function formatCurrency(value) {
    return new Intl.NumberFormat('en-US', {
        ...
        minimumFractionDigits: 0,
        maximumFractionDigits: 0
    }).format(Math.round(value)).replace('$', '');
}
